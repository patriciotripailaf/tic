<?php
    ...
    function userLogin($usuario,$password){
        ...
        $sql = "SELECT idjugador,status,usuario FROM jugador where usuario = '".$usuario."' AND password = '".$password."'";
        $qry = $this->db->query($sql);
        $resultado = $qry->row_array();
        if($resultado!=null){
            return $resultado;
        ...
        $sql = "UPDATE jugador SET ultimo_ingreso=".date('Y-m-d H:i:s')."WHERE idjugador=".$idJugador;
        $qry = $this->db->query($sql);
